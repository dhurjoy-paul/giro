...
export default function OverviewSection() {
  return (
    <section className="relative z-0 py-24 px-6 sm:px-10 md:px-16 lg:px-24 bg-transparent text-text overflow-hidden">

      {/* blob */}
      <div aria-hidden="true"
        className="absolute top-0 left-1/2 -z-10 -translate-x-1/2 blur-3xl opacity-20"
      >
        <div className="aspect-[1155/678] w-[72rem] bg-gradient-to-tr from-amber-200 to-emerald-200"
          style={{
            clipPath:
      ...
      <div className="text-center mb-16 max-w-3xl mx-auto">
        <h2 className="text-4xl sm:text-5xl font-bold font-bricolage-grotesque">Why Choose GIRO?</h2>
          ...
          <Tab.List className="space-y-4">
            {overviewTabs.map((tab, idx) => (
              <Tab key={idx} as={Fragment}>
                {({ selected }) => (
                  <button
                    className={`w-full text-left px-6 py-5 rounded-2xl border transition duration-200 focus:outline-none ${selected
                  ...
                  >
                    <h3 className="text-xl font-semibold mb-1 font-bricolage-grotesque">{tab.title}</h3>
